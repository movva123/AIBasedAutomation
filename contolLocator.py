...
def control_box_text(img,detection_box):
    ...
    for i in detection_box:
        if i[0] !=0:
            ...
            yR=int(i[2]*height)
            xR=int(i[3]*width)
            control=img.crop([xL-1,yL,xR,yR])
            #contrast=ImageEnhance.Contrast(control)
            #img=contrast.enhance(3)
            control.show()
            coord=(xL,yL,xR,yR)
            controlname=pytesseract.image_to_string(control)
            print(controlname,'located at',coord)
...
for image_path in TEST_IMAGE_PATHS:
  ...
  control_box_text(image,output_dict['detection_boxes'])
